"""Test file for general testing of the eBay
API interactions and associated views"""
...
# pylint goes crazy for this and it conflicts with
# actually maintaining the tests well
import os
...
class EbayViewsTestCase(TestCase):
    ...
    def test_challenge_code_response(self):
        ...
        response = self.client.get(reverse('ebay_market_delete') +
                                   '?challenge_code=test123')
...
-----END PUBLIC KEY-----"""
        mock_get.return_value = MagicMock(status_code=200,
                                          json=lambda: {'key': fake_public_key})

        body = json.dumps({'dummy': 'data'}).encode('utf-8')
        signature_dict = {'kid': 'fake_kid',
                          'signature':
                          base64.b64encode(b'bad_signature').decode('utf-8')}
        headers = {'HTTP_X_EBAY_SIGNATURE':
                   base64.b64encode(
                       json.dumps(signature_dict).encode('utf-8')
                   ).decode('utf-8')}

        response = self.client.post(reverse('ebay_market_delete'),
                                    data=body,
    ...
    @patch('api.views.get_oath_token', return_value='fake_token')
    @patch('requests.get')
    def test_ebay_get_items_post_parses_items_correctly(self,
                                                        mock_get, _mock_token):
        """Test POST /ebay_add_items/ parses items into context without
        DB save"""
        ...
        mock_detail_response = {"shortDescription":
                                "Test description",
                                "seller": {"sellerId": "seller1"},
                                "image": {"imageUrl":
        ...
        response = self.client.post(reverse('ebay_get_items'),
    ...
    @patch('api.views.get_oath_token', return_value='fake_token')
    def test_ebay_deletion_removes_items_with_matching_seller_id(
            self, _mock_get_oauth, mock_requests_get, mock_load_key):
        """Ensures deletion webhook removes Items with matching seller_id"""
        item1 = Item.objects.create(title="Item A", description="desc",
                                    image_url="http://img/a",
                                    tag="Accessory",
                                    item_id="111", seller_id="delete_me")
        item2 = Item.objects.create(title="Item B", description="desc",
                                    image_url="http://img/b", tag="Accessory",
                                    item_id="222", seller_id="keep_me")

        mock_requests_get.return_value.status_code = 200
        mock_requests_get.return_value.json.return_value = {"key":
 """-----BEGIN PUBLIC KEY-----
 fake
 -----END PUBLIC KEY-----"""
                                                            }
        ...
        fake_sig = base64.b64encode(json.dumps({"kid": "fake",
                                                "signature":
                                                base64.b64encode(
                                                    b'abc').decode()}
                                               ).encode()).decode()
    ...
    @patch('api.views.get_oath_token', return_value='fake_token')
    def test_invalid_public_key_returns_412(self, _mock_token,
                                            _mock_load_key, mock_requests_get):
        """
    ...
    @patch('api.views.get_oath_token', return_value='fake_token')
    def test_public_key_fetch_exception_returns_500(self, _mock_get_oauth,
                                                    _mock_requests_get,
                                                    _mock_load_key):
    ...
    @patch('api.views.serialization.load_pem_public_key')
    @patch('api.views.get_oath_token', return_value='fake_token')
    def test_missing_username_in_payload(self, _mock_token,
                                         mock_load_key, mock_requests_get):
        """
        Branch: payload missing 'username' field
        """
        # mock public key returns a valid key
        # (so we pass signature verification)
        mock_requests_get.return_value.status_code = 200
        mock_requests_get.return_value.json.return_value = {
            "key":
 """-----BEGIN PUBLIC KEY-----
 ...
 -----END PUBLIC KEY-----"""
        ...
        # Mock load_pem_public_key to return a
        # fake key object whose verify() does nothing
        fake_key = MagicMock()
        fake_key.verify = MagicMock(return_value=None)
        mock_load_key.return_value = fake_key
        ...
        # Optionally check the log or response content
        # if you modify the view to return a message
...
class EbaySignatureValidationSuccessTest(TestCase):
    ...
    def setUp(self):
        ...
        self.fake_body = json.dumps({"user_id": "12345",
                                     "action": "delete"}).encode("utf-8")
        signature = self.private_key.sign(self.fake_body,
                                          ec.ECDSA(hashes.SHA1()))
        encoded_signature = base64.b64encode(signature).decode("utf-8")
        header_json = json.dumps({"kid": "test_key_id",
    ...
    def test_valid_signature_returns_200(self, _mock_token, mock_get):
        ...
        request = self.factory.post("/auth/ebay_market_delete/",
                                    data=self.fake_body,
                                    content_type="application/json",
