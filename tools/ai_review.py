...
This module provides functionality to fetch pull request diffs from GitHub,
generate AI-powered code reviews using OpenAI, and post the reviews back
to the pull request as comments.
"""
import os
import github
import openai
...
def get_pull_request_diff(
    ...
    if resp.status_code == 404:
        raise RuntimeError(
            f"Pull request {pr_number} or repository "
            f"{repo_full_name} not found"
        )
...
def post_pull_request_comment(
        token: str, repo_full_name: str, pr_number: int, comment: str
) -> str:
    """
    ...
    try:
        ...
        return (getattr(created, "html_url", None) or
        ...
        raise RuntimeError(
            f"GitHub API error posting comment: {msg}"
        ) from e
...
def generate_ai_review(
        openai_client, diff: str, model: str = "gpt-4"
) -> str:
    """
    Generate an AI review of the given pull request diff using the
    specified model.
    ...
    # Include the system instruction in the prompt since prompt_ai
    # sends a single user message.
    prompt = (
        "You are an expert code reviewer. Please provide a thorough "
        "review of the following pull request diff. "
        ...
        "Provide potential fixes or improvements where applicable. "
        "Potential fixes and improvements will include code snippets "
        "and examples.\n\n"
        f"Pull Request Diff:\n{diff}"
    )
    ...
    try:
        ...
        raise RuntimeError(
            f"OpenAI API error generating review: {e}"
        ) from e
...
def main():
    ...
    Reads configuration from environment variables and executes the review.
    """
    ...
    comment_url = post_pull_request_comment(
        github_token, repo_full_name, pr_number, review
    )
